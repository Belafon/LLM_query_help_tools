...
if (fs.existsSync(SETTINGS_FILE)) {
  try {
    settings = readJsonFileSync(SETTINGS_FILE);
  } catch (e) {
...
function getGlobalPaths() {
  if (fs.existsSync(GLOBAL_PATHS_FILE)) {
    try {
      return readJsonFileSync(GLOBAL_PATHS_FILE);
    } catch (e) {
...
function getSecretKeys() {
  if (fs.existsSync(SECRET_KEYS_FILE)) {
    try {
      return readJsonFileSync(SECRET_KEYS_FILE);
    } catch (e) {
...
function getSecretValues() {
  if (fs.existsSync(SECRET_VALUES_FILE)) {
    try {
      return readJsonFileSync(SECRET_VALUES_FILE);
    } catch (e) {
...
app.post('/api/execute', async (req, res) => {
  ...
  try {
    ...
    const unreplacedVars = findUnreplacedVariables(processedScript);
    if (unreplacedVars.length > 0) {
      console.warn(`WARNING: Script "${scriptName}" has unresolved template variables: ${unreplacedVars.join(', ')}`);
      console.warn('These variables are not defined in Path Manager (global_paths.json) or Secret Manager (secrets/values.json).');
    }
...
`;
  ...
  // Check for unreplaced template variables in the script
  const unreplacedVars = findUnreplacedVariables(processedScript);
  let unreplacedWarningPs = '';
  if (unreplacedVars.length > 0) {
    const varList = unreplacedVars.map(v => `{{${v}}}`).join(', ');
    unreplacedWarningPs = `
Write-Host "" 
Write-Host "========== WARNING: UNRESOLVED TEMPLATE VARIABLES =========" -ForegroundColor Red
Write-Host "The following template variables were NOT replaced:" -ForegroundColor Red
Write-Host "  ${varList}" -ForegroundColor Yellow
Write-Host "" -ForegroundColor Red
Write-Host "These variables are not defined in Path Manager or Secret Manager." -ForegroundColor Red
Write-Host "PowerShell interprets unreplaced {{VAR}} as script blocks," -ForegroundColor Red
Write-Host "which causes 'Cannot evaluate parameter' errors." -ForegroundColor Red
Write-Host "" -ForegroundColor Red
Write-Host "To fix: Add these aliases in the Path Manager page" -ForegroundColor Cyan
Write-Host "        or add secrets in the Secret Manager page." -ForegroundColor Cyan
Write-Host "==========================================================" -ForegroundColor Red
Write-Host ""
throw "Script aborted: unresolved template variables: ${varList}"
`;
  }

  // Write user script to a separate file to avoid template interpolation issues
  // Prepend warning about unreplaced variables if any were found
  fs.writeFileSync(userScriptPath, unreplacedWarningPs + processedScript, 'utf8');
...
/**
 * Checks for any remaining unreplaced {{VARIABLE}} placeholders in the script content.
 * Returns an array of variable names that were not resolved.
 */
function findUnreplacedVariables(scriptContent) {
  const matches = scriptContent.match(/\{\{([^}]+)\}\}/g);
  if (!matches) return [];
  // Extract unique variable names (without the {{ }})
    ...
    if (fs.existsSync(WORKSPACES_DIR)) {
      ...
      for (const workspaceName of workspaces) {
        ...
        if (fs.existsSync(scriptsPath)) {
          try {
            const scriptsData = readJsonFileSync(scriptsPath);
...
async function handleScriptExecution(ws, data) {
  ...
  try {
    ...
    // Check for unreplaced template variables and warn the user
    const unreplacedVars = findUnreplacedVariables(processedScript);
    if (unreplacedVars.length > 0) {
      const varList = unreplacedVars.map(v => `{{${v}}}`).join(', ');
      console.warn(`WARNING: Script "${scriptName}" has unresolved template variables: ${varList}`);
      ws.send(JSON.stringify({
        type: 'warning',
        sessionId,
        message: `Warning: The following template variables are not defined and were NOT replaced: ${varList}. ` +
          `Please add them in Path Manager (global_paths.json) or Secret Manager. ` +
          `Unresolved {{VAR}} placeholders will cause PowerShell errors because curly braces are interpreted as script blocks.`
      }));
    }
  ...
  try {
    if (dataType === 'paths') {
      ...
      if (fs.existsSync(scriptsFile)) {
        allData = readJsonFileSync(scriptsFile);
      }
...
function startAutoStartScripts() {
  ...
  if (fs.existsSync(scriptsFile)) {
    try {
      const scriptsData = readJsonFileSync(scriptsFile);
